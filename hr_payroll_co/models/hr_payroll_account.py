...
class HrPayslip(models.Model):
    ...
    def _action_create_account_move(self):
        #print('------------------- _action_create_account_move en hr_payroll_co -------------------')
                ...
                if float_compare(credit_sum, debit_sum, precision_digits=precision) == -1:
                    ...
                    if not adjust_credit:
                        adjust_credit = {
                            ...
                            'partner_id': slip.employee_id.work_contact_id and slip.employee_id.work_contact_id.id or False,
                            'account_id': acc_id,
    ...
    def _prepare_line_values(self, line, account_id, date, debit, credit):
        analytic_account_id = line.salary_rule_id.analytic_account_id or line.slip_id.contract_id.analytic_account_id
        if not analytic_account_id:
           raise UserError('No se ha definido una cuenta analítica al empleado %s' % (line.partner_id.name,))

        partner_entidad = False
        if debit != 0 and line.salary_rule_id.debit_partner:
           setting_id = self.env['hr.contract.parameter.value'].sudo().search([('contract_id','=',line.contract_id.id),('rule_parameter_id','=',line.salary_rule_id.debit_partner.id)]) 
           if not setting_id:
              raise UserError('No parametrizado la entidad asociada al parámetro %s en el contrato de %s' % (line.salary_rule_id.debit_partner.name, line.contract_id.name,))
           
           partner_entidad = setting_id[0].parameter_partner_id.id

        if credit != 0 and line.salary_rule_id.credit_partner:
           setting_id = self.env['hr.contract.parameter.value'].sudo().search([('contract_id','=',line.contract_id.id),('rule_parameter_id','=',line.salary_rule_id.credit_partner.id)]) 
           if not setting_id:
              raise UserError('No parametrizado la entidad asociada al parámetro %s en el contrato de %s' % (line.salary_rule_id.credit_partner.name, line.contract_id.name,))

           partner_entidad = setting_id[0].parameter_partner_id.id

        return {
            'name': line.name,
            'partner_id': partner_entidad or (line.partner_id and line.partner_id.id) or (line.employee_id.work_contact_id and line.employee_id.work_contact_id.id),
            'account_id': account_id,
            'journal_id': line.slip_id.struct_id.journal_id.id,
            'date': date,
            'debit': debit,
            'credit': credit,
            'analytic_distribution': {str(analytic_account_id.id): 100},
        }
