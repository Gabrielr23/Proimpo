...
class HrContractParameterValue(models.Model):
    ...
    _sql_constraints = [
        ('_unique', 'unique (contract_id, rule_parameter_id, date_from)', "Ya existe este parámetro para este contrato"),
    ]
...
class HrContract(models.Model):
    ...
    setting_ids = fields.One2many(
        'hr.contract.parameter.value', 
        'contract_id',
        string=u'Configuración',
    )
    area_contable = fields.Selection([
        ('A', 'Administrativa'),
        ('C', 'Comercial'),  
        ('O', 'Operativa'),        
    ], string='Area contable', required=True, default='A')       
