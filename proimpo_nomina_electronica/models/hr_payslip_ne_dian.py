...
class HrPayslip(models.Model):
    ...
    def _ne_add_signature_node(self, root, cert):
        ...
        uuid_str = str(xml_utils._uuid1())
        doc_id = "xmldsig-" + uuid_str
        # IMPORTANTE: el Anexo Técnico (sección 3.6, regla DC10) exige literalmente
        # URI="#{UUID}-KeyInfo" para la referencia a KeyInfo -- UUID PELADO (sin el
        # prefijo 'xmldsig-') y con mayúsculas en "KeyInfo". Es un patrón DISTINTO al
        # de SignedProperties (regla DC13: '#xmldsig-{UUID}-signedprops', que sí lleva
        # el prefijo y va en minúsculas). Antes ambos usaban el mismo patrón
        # (doc_id + sufijo en minúsculas), lo cual no coincidía con lo exigido para
        # KeyInfo y es la causa más probable del rechazo ZE02 pese a que la firma
        # verificaba correctamente con herramientas genéricas (xmlsec1), ya que estas
        # solo validan consistencia interna Id/URI, no el patrón literal exacto.
        keyinfo_id = uuid_str + "-KeyInfo"
