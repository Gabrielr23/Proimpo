        ...
        for liq in self:
            ...
            if liq.date_liquidacion and liq.employee_id:
               ...
               if not liq.contract_id:
                  raise ValidationError('No se encuentra un contrato activo para el empleado gcontrat %s' % (liq.employee_id.name,))
...
class HrContractLiquidationDetails(models.Model):
    ...
    def _calculo_campos(self):
        ...
        for line in self:
            ...
            line.days_average = dias360(line.date_from, line.date_to)
            ...
            if line.liquidation_type == 'V':
               ...
               line.days_total = dias360(line.date_from, line.date_to) 
            ...
            if line.liquidation_type == 'P':
               ...
               cesantias = round((line.total_average * line.days_neto)/360) 
               line.amount = round((cesantias * line.days_neto * 0.12)/360)
               ...
               if line.liquidation_id.type_contract == 'F':
                  ...
                  dias_terminacion = dias360(line.date_to, line.liquidation_id.contract_end_date)
                  line.date_from = line.date_to
                  ...
                  dias_laborados = dias360(line.liquidation_id.date_start, line.date_to)
    ...
    def compute_holidays_pending(self):
        ...
        for record in self:
            ...
            dias_totales = dias360(record.date_from, record.date_to)
            print('dias_totales: ',dias_totales)
            ...
            for vac in leave_ids_vc_entre:
                if vac.state == 'validate':
                   ...
                   f_hasta = record.date_to

                  
                   # Determina si el empleado trabaja el sábado como hábil
                   sabado = record.employee_id.sabado
                   diff_day = 0    
