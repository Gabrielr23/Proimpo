    ...
    #def _reg_encabezado(self): 
        ...
        for record in self.env['account.payment'].browse(active_ids):
            ...
            if record and record.file_status == 'generado':
                raise UserError(_('Algunos comprobantes ya han sido generados'))
        ...
        for record in self.env['account.payment'].browse(active_ids):
            if record and record.file_status == 'no_generado':
                ...
                text_reg_2 = '6' + id_partner.rjust(15,'0') + name_partner[0:18].ljust(18,' ') + cod_bank.ljust(9,'0') + cta_partner.rjust(17, "0") + 'S' + transaccion + valor.zfill(10) + record.name[0:9] + comment[0:12] + ' '  
                ...
                #if record and rec.file_status == 'no_generado':
