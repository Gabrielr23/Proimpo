...
class PaymentExportWizard(models.TransientModel):
    ...
    #def _reg_encabezado(self): 
        ...
        nit = str(company_id.ref)
        name_company = company_id.name
        num_reg = str(num_reg)
        total = str(total)
        total = total.replace('.','')

        print('nit...........',type(cuenta_debitar),type(t_c))
        
        
        text_reg_1 = '1' + nit.zfill(10) + name_company.ljust (16," ") + self.transaccion + self.descripcion.ljust(10, " ") + fecha_pago + self.sequence + fecha_apli + str(count).zfill(6) + (12 * '0') + total.zfill(12) + str(cuenta_debitar).zfill(11) + tipo_cta 
        ...
        for record in self.env['account.payment'].browse(active_ids):
            if record:
                ...
                id_partner = str(record.partner_id.ref)
                ...
                text_reg_2 = '6' + id_partner.rjust(15,'0') + name_partner[0:18] + cod_bank.rjust(9,'0') + cta_partner.ljust(17, "0") + 'S' + transaccion + valor.zfill(10) + record.name[0:9] + record.ref[0:9] + ' '  
