...
class PaymentExportWizard(models.TransientModel):
    ...
    #def _reg_encabezado(self): 
        ...
        file.close()
        file_down = '/opt/odoo15/odoo-server/addons-extra/report_sql/static/'+filename
        data = open(file_down, "rb").read()
        base_url = self.env['ir.config_parameter'].get_param('web.base.url')
        attachment_obj = self.env['ir.attachment']
        # create attachment
        attachment_id = attachment_obj.create({'name': "name", 'datas_fname': 'name.file_ext', 'datas': data, 'mimetype': 'text/plain'})
