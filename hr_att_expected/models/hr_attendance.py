...
class HrAttendance(models.Model):
   ...
   # Campo para horas extras aprobadas
    ...
    def _get_work_intervals_from_planning(self, employee, att_date, tz):
        """
        Devuelve los intervalos de trabajo (start_utc, end_utc) tomados del
        planning.slot del empleado para esa fecha. Lista vacía si no hay slot.

        Mismo formato que _get_work_intervals_from_calendar: tuplas en UTC
        sin timezone info.
        """
        slot_data = self._get_planning_slot(employee, att_date, tz)
        if not slot_data:
            return []
    ...
    def _get_work_intervals(self, employee, att_date, tz):
        """
        Intervalos de trabajo a usar en TODOS los cómputos de horas.

        Regla de negocio: usar el turno planificado (planning.slot) si existe;
        de lo contrario, caer al horario del resource.calendar.

        Esto garantiza que HO/HRN/HDES/extras se calculen contra el mismo
        turno que define expected_check_in / expected_check_out.
        """
        intervals = self._get_work_intervals_from_planning(employee, att_date, tz)
        if intervals:
            return intervals
        ...
        if not calendar:
            return []
        return self._get_work_intervals_from_calendar(calendar, att_date, tz)
    ...
    def _round_to_quarter_hour(self, hours):
        ...
        IMPORTANTE: Si el valor está muy cerca de una hora completa (falta < 1 minuto),
        ...
        # OJO: el umbral debe ser (3600 - 60)/3600 = 0.98333 (falta < 1 min),
        # NO (3600 - TOLERANCE*60)/3600, que con TOLERANCE=15 daría 0.75 (45 min)
        # y dispararía el redondeo a hora entera demasiado pronto.
        fractional_part = hours % 1
        if fractional_part >= (3600 - 60) / 3600:  # >= ~59 minutos
    ...
    def _compute_discounted_hours(self):
        ...
        Aplica un margen de tolerancia para llegadas tarde y salidas tempranas.
        ...
        for rec in self:
            ...
            # Obtener intervalos de trabajo (planning si existe, si no calendario)
            tz = self._get_employee_tz(rec.employee_id)
            check_in_local = pytz.UTC.localize(rec.check_in).astimezone(tz)
            att_date = check_in_local.date()
